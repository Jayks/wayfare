  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500">{trips.length} trips created across the platform</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="glass rounded-2xl p-4">
            ...
            <p className="text-2xl font-semibold text-slate-800 tabular">{s.value}</p>
          </div>
        ))}
      </div>
        ...
        rows.length > 0 && (
          <div key={label}>
            <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3 flex items-center gap-2">
              ...
              {label}
              <span className="text-slate-300 font-normal normal-case tracking-normal">· {rows.length}</span>
            </h2>
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-slate-50/40 transition-colors">
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <p className="font-medium text-slate-800 truncate max-w-[180px]">{t.name}</p>
                          ...
                          <td className="px-5 py-3 hidden md:table-cell">
                            <p className="text-slate-600 truncate max-w-[140px]">{t.creatorName ?? "—"}</p>
                          </td>
                          ...
                          <td className="px-5 py-3 text-right tabular hidden lg:table-cell">
                            {spend > 0 ? (
                              <span className="font-medium text-slate-700" style={{ fontFamily: "var(--font-fraunces)" }}>
      ...
      <div className="glass rounded-xl px-5 py-4 flex flex-wrap items-center gap-6">
        <div className="flex items-center gap-2 text-sm text-slate-600">
          ...
          <span className="font-semibold text-slate-800 tabular" style={{ fontFamily: "var(--font-fraunces)" }}>
            {formatCurrency(totalSpend, "INR")}
          </span>
          <span className="text-slate-400 text-xs">(all currencies summed as INR)</span>
        </div>
      </div>
    </div>
  );
