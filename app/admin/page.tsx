  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500">Overview of all users, trips, and activity.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((s) => (
          <div key={s.label} className="glass rounded-2xl p-4">
            ...
            <p
              className="text-2xl font-semibold text-slate-800 tabular"
          ...
          {[
            ...
            return (
              <div key={role} className="glass rounded-2xl overflow-hidden">
                <div className="flex items-center gap-3 px-5 py-3 border-b border-white/50">
                  ...
                  <span className="ml-auto text-xs font-medium text-slate-600">{roleUsers.length} users</span>
                </div>
                {roleUsers.length === 0 ? (
                  <p className="px-5 py-4 text-xs text-slate-400 italic">No users in this tier</p>
                ) : (
                    ...
                    {roleUsers.slice(0, 10).map((u) => (
                      <div key={u.id} className="flex items-center gap-3 px-5 py-2.5">
                        ...
                        <div className="text-right shrink-0 hidden sm:block">
                          <p className="text-xs text-slate-500">{u.tripsOwned} trips owned</p>
                          <p className="text-xs text-slate-400">{u.tripsJoined} total trips</p>
                        </div>
                        <p className="text-xs text-slate-400 shrink-0">{formatDate(u.joinedAt)}</p>
                      </div>
                    ))}
                    {roleUsers.length > 10 && (
                      <p className="px-5 py-2 text-xs text-slate-400">
                        +{roleUsers.length - 10} more ·{" "}
                        <a href="/admin/users" className="text-cyan-600 hover:underline">View all</a>
                      </p>
                    )}
                  </div>
                )}
        ...
        <div className="glass rounded-2xl overflow-hidden">
                ...
                <tr key={t.id}>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2">
                      <p className="font-medium text-slate-700 truncate max-w-[200px]">{t.name}</p>
                      {t.isArchived && (
                        <span className="text-xs bg-slate-100 text-slate-400 px-1.5 py-0.5 rounded">archived</span>
                      )}
                    </div>
                  </td>
                  <td className="px-5 py-3 text-right tabular text-slate-600 hidden sm:table-cell">
                    {Number(t.memberCount)}
                  </td>
                  <td className="px-5 py-3 text-right tabular text-slate-600 hidden md:table-cell">
          ...
          {trips.length > 15 && (
            <div className="px-5 py-3 border-t border-slate-100 text-xs text-slate-400">
              ...
              <a href="/admin/trips" className="text-cyan-600 hover:underline">View all</a>
            </div>
          )}
        </div>
      </div>
